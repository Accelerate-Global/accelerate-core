...
2. Analyze the existing codebase thoroughly - architecture patterns, technical constraints, integration points. Ground all recommendations in what you actually observe, not assumptions about how systems typically work.
  Before proposing architecture for a specific area, read:
  - The repository-level `AGENTS.md`
  - Any more specific `AGENTS.md` files in the affected subtree
   When multiple `AGENTS.md` files apply, use the more specific guidance for that area.
  If the workflow seed or applicable `AGENTS.md` guidance identifies required skills, treat those skills as implementation constraints that must be carried forward into later execution and review.
...
- The proposed technical direction is grounded in the actual codebase and applicable `AGENTS.md` guidance
